...
d3.json(queryUrl).then(function (data) {
  // Creating GeoJSON layer that contains the features array on the earthquakeData object.
  L.geoJSON(data, {
  ...
  function onEachFeature(feature, layer) {
    layer.bindPopup(
      ...
      )}</p><hr><p>Magnitude: ${feature.properties.mag}</p><hr><p>Depth: ${
  ...
  //Define function to select marker size
...
legend.onAdd = function () {
  ...
  for (let i = 0; i < depthRange.length; i++) {
    div.innerHTML +=
      ...
      " '></i>" +
      depthRange[i] +
      (depthRange[i + 1] ? "&ndash;" + depthRange[i + 1] + "<br>" : "+");
  }
  return div;
};
//Add legend to map
